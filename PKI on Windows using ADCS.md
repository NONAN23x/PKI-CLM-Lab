...
2.  [VirtualBox Setup](#virtualbox-setup)
    *   [Step 1: Create the Virtual NAT Network](#step-1-create-the-virtual-nat-network)
    *   [Step 2: Create the Primary DC Virtual Machine](#step-2-create-the-primary-dc-virtual-machine)
    *   [Step 3: Create the Secondary Server Virtual Machine](#step-3-create-the-secondary-server-virtual-machine)
3.  [Primary Domain Controller Setup](#primary-domain-controller-setup)
    *   [Step 1: Install Windows Server 2025](#step-1-install-windows-server-2025)
    *   [Step 2: Configure Network Settings](#step-2-configure-network-settings)
    *   [Step 3: Install Active Directory Domain Services](#step-3-install-active-directory-domain-services)
    *   [Step 4: Promote to Domain Controller](#step-4-promote-to-domain-controller)
    *   [Step 5: Verify Active Directory Installation](#step-5-verify-active-directory-installation)
4.  [Certificate Services Setup](#certificate-services-setup)
    *   [Step 1: Install Active Directory Certificate Services](#step-1-install-active-directory-certificate-services)
    *   [Step 2: Configure ADCS](#step-2-configure-adcs)
    *   [Step 3: Create a Certificate Template for User Enrollment](#step-3-create-a-certificate-template-for-user-enrollment)
    *   [Step 4: Verify ADCS Installation](#step-4-verify-adcs-installation)
5.  [Second Machine Setup](#second-machine-setup)
    *   [Step 1: Install Windows Server 2025 on SRV01](#step-1-install-windows-server-2025-on-srv01)
    *   [Step 2: Configure Network Settings on SRV01](#step-2-configure-network-settings-on-srv01)
    *   [Step 3: Join SRV01 to the Domain](#step-3-join-srv01-to-the-domain)
    *   [Step 4: Verify Domain Membership](#step-4-verify-domain-membership)
6.  [Post-Configuration Tasks](#post-configuration-tasks)
    *   [Task 1: Configure User Accounts](#task-1-configure-user-accounts)
    *   [Task 2: Create Organizational Units (OUs)](#task-2-create-organizational-units-ous)
    *   [Task 3: Configure Group Policy](#task-3-configure-group-policy)
    *   [Task 4: Request a Certificate on SRV01](#task-4-request-a-certificate-on-srv01)
    *   [Task 5: Configure SSL for Web Services (Optional)](#task-5-configure-ssl-for-web-services-optional)
    *   [Task 6: Enable File Server Role on SRV01 (Optional)](#task-6-enable-file-server-role-on-srv01-optional)
7.  [Useful Commands and Tools](#useful-commands-and-tools)
    *   [PowerShell Commands](#powershell-commands)
    *   [MMC Tools to Add](#mmc-tools-to-add)
8.  [Troubleshooting](#troubleshooting)
    *   [Issue: SRV01 Cannot Join Domain](#issue-srv01-cannot-join-domain)
    *   [Issue: Certificate Enrollment Fails](#issue-certificate-enrollment-fails)
    *   [Issue: VMs Lose Connectivity After Restart](#issue-vms-lose-connectivity-after-restart)
    *   [Issue: DC Boot Hangs After Promotion](#issue-dc-boot-hangs-after-promotion)
9.  [Next Steps](#next-steps)
    *   [Advance Your Lab](#advance-your-lab)
    *   [Lab Maintenance](#lab-maintenance)
10. [Security Best Practices](#security-best-practices)
11. [Reference URLs](#reference-urls)
12. [Lab Configuration Summary](#lab-configuration-summary)
